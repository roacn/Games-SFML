...
void SwagBall::initShape(const sf::RenderWindow& window)
{
    ...
    this->shape.setPosition(sf::Vector2f(
        static_cast<float>(rand() % (window.getSize().x - static_cast<int>(this->shape.getGlobalBounds().width))),
        static_cast<float>(rand() % (window.getSize().y - static_cast<int>(this->shape.getGlobalBounds().height)))));
}
